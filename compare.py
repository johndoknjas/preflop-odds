...
def getHandRankFromFiveCards(fC: list[int], fS: list[int]):
    ...
    if fS.count(fS[0]) == len(fS):
        ...
        (
            fC.extend([7, fC[0], fC[4]])
            if fC[0] == fC[1]
            else fC.extend([7, fC[4], fC[0]])
        )  # quads
        ...
        (
            fC.extend([3, fC[1], fC[4], fC[0]])
            if fC[1] == fC[2]
            else fC.extend([3, fC[2], fC[1], fC[0]])
        )
    elif (fC[0] == fC[1] and (fC[2] == fC[3] or fC[3] == fC[4])) or (
        fC[1] == fC[2] and fC[3] == fC[4]
    ):  # two pair
        if fC[0] == fC[1] and fC[2] == fC[3]:
            fC.extend([2, fC[3], fC[1], fC[4]])  # kicker higher than both pairs
        else:
            (
                fC.extend([2, fC[4], fC[1], fC[2]])
                if fC[0] == fC[1] and fC[3] == fC[4]
                else fC.extend([2, fC[4], fC[1], fC[0]])
            )
    elif fC[0] == fC[1] or fC[1] == fC[2]:
        (
            fC.extend([1, fC[0], fC[4], fC[3], fC[2]])
            if fC[0] == fC[1]
            else fC.extend([1, fC[1], fC[4], fC[3], fC[0]])
        )
    elif fC[2] == fC[3] or fC[3] == fC[4]:
        (
            fC.extend([1, fC[2], fC[4], fC[1], fC[0]])
            if fC[2] == fC[3]
            else fC.extend([1, fC[3], fC[2], fC[1], fC[0]])
        )
    return (
        fC if len(fC) > 5 else fC + [0]
